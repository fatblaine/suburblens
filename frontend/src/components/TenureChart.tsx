        ...
        {YEARS.map(({ key, label }, idx) => {
          ...
          return (
            <div key={key}>
              ...
              {/* Mobile: the side percentage column is hidden, so show the
                  read-outs under the bar (colour matches the legend above). */}
              <div className="flex sm:hidden gap-4 pl-16 mt-2 font-mono text-xs">
                {SLICES.map(s => {
                  const pct = tenure[s.key][key] ?? 0
                  return (
